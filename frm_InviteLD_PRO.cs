...
{
    public partial class frm_InviteLD_PRO : Form
    {
        public frm_InviteLD_PRO(List<Account> list_acc, frm_MainLD_PRO frm_main)
        {
            InitializeComponent();
        ...
        private void frmInviteLD_Load(object sender, EventArgs e)
        {
            method_LoadAccount();
            method_Config();
        ...
        private void btnRun_Click(object sender, EventArgs e)
        {
            ...
            if (rdoLikePage.Checked)
            {
                if (txt_namepage.Text == "")
                {
                    MessageBox.Show("Hãy nhập Id của Page");
                    return;
                }
            }
            if (rdoLikeGroup.Checked)
            {
                if (txtGoupsId.Text == "")
                {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (status)
                    {
                        ...
                        dr.Cells["Message"].Value = "Login successfull";
                        acc.TrangThai = "Live";
                        ...
                        if (rdoLikePage.Checked)
                        {
                            u.setStatus(ldID, "Invite friend like Page...");
                            dr.Cells["Message"].Value = "Invite friend like Page";
                            message += ld.loopviewInviteLikePage(ldID, acc.app, (int)numInvitePage.Value, txt_namepage.Lines.ToList(), delay, chkLoopInvitePage.Checked, (int)numLoopPage.Value, token);
                        }
                        else if (rdoLikeGroup.Checked)
                        {
                            u.setStatus(ldID, "Invite friend join group...");
                            dr.Cells["Message"].Value = "Invite friend join group";
                            message += ld.viewInviteGroup(u,acc,ldID, acc.app, (int)numInviteGroup.Value, txtGoupsId.Lines.ToList(), delay, token);
                        }
                        #endregion
        ...
        {
            rdoLikePage.Checked = false;
            rdoLikeGroup.Checked = false;
            switch (tabControl.SelectedIndex)
            {
                case 0:
                    rdoLikePage.Checked = true;
                    break;
                case 1:
                    rdoLikeGroup.Checked = true;
                    break;
            }
        }
        ...
        {
            this.Text = "Invite friend join, follow";
            rdoLikePage.Text = "Like, follow page";
            rdoLikeGroup.Text = "Join Group";
            label4.Visible = false;
            label26.Text = "Friends / 1 time";
            label27.Text = "1 ID / 1 line";
            label30.Text = "Friends / 1 time";
            label6.Text = "1 ID / 1 line";
            label2.Text = "seconds";
            chkLoopInvitePage.Text = "Loop action invite page";
            label3.Text = "times";
        }
