    ...
    public partial class frm_Login : Form
    {
            ...
            if (result.status)
            {
                ...
                if (cusrequest.Random == cus.Random && cusrequest.Email == cus.Email)
                {
                    ...
                    if (SettingTool.note != null)
                    {
                        ...
                        {
                            ResultRequest kq = customercontroller.method_Update(22);
                            if (kq.status)
                            {
        ...
        private void method_OpenMain(Object obj)
        {
            try
            {
                ...
                frm_MainLD frmLD = new frm_MainLD(li);
                frmLD.ShowDialog();
        ...
        private void frmLogin_Load(object sender, EventArgs e)
        {
            ...
            up.updateData23();
            up.updateDataColName();
            up.updateDataDanhmuc();
            up.updateDataColId();
            up.updateDisplay();
            up.updateName();
            up.updatePathPost();
            up.updateLDID();
            up.updatePathUID();
            up.createTableGroupLD();
            up.createTableDetailLD();
            up.createColumnProxy();
            up.createColumnFriend();
            up.createColumnToken();
            up.createColumnPathCover();
            up.createColumnKeyVPN();
            up.createColumnKeyUseragent();
            up.createColumnKeyLastRun();
            up.createColumnKeyDataPost();
            up.createColumnKeyBirthday();
            up.createColumnDNS();
            up.createColumnCity();
            up.createColumnDNSAccount();

            //khach vip the hien dns,proxy
            if (File.Exists(String.Format("{0}\\Config\\{1}.txt", Application.StartupPath, "ListUser")))
            {
                Data dt = new Data();
                dt.method_DeleteData("update Account set proxy = (SELECT proxy from DetailLD WHERE ldid = Account.ldid), dns = (SELECT dns from DetailLD WHERE ldid = Account.ldid)");
            }
        }
