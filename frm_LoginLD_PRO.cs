...
{
    public partial class frm_LoginLD_PRO : Form
    {
        public frm_LoginLD_PRO(frm_MainLD_PRO frm_main, List<Account> list_acc)
        {
            InitializeComponent();
        ...
        private void method_Start(string ldID, DataGridViewRow dr, string proxy, CancellationToken token)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    bool haslogin = ld.logoutloginFacebookLD(u, acc, token);
                    if (haslogin)
                    {
                        if (stop)
                        {
