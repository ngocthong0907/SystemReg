...
{
    public partial class frm_regnick_email : Form
    {
        public frm_regnick_email(frm_MainLD_PRO frm_main)
        {
            InitializeComponent();
        ...
        {
            ls_email = File.ReadAllLines(txtEmail.Text).ToList();
            regnick regcf = new regnick();
        ...
        public List<string> ls_email = new List<string>();
        ...
        private void method_Start(string ldID, DataGridViewRow dr, string proxy, CancellationToken token)
        {
                ...
                while (success < total)
                {
                    ...
                    if (openLD(u, ldID, dr, acc, token))
                    {
                        ...
                        try
                        {
                            ...
                            lock (synlock)
                            {
                                string email = ls_email[0];
                                ls_email.RemoveAt(0);
                                string[] arrEmail = email.Split('|');
                                acc.email = arrEmail[0];
                                acc.dataprofile = arrEmail[1];

                            }
                            result_reg = ld.regnick_Email(acc, dr, u, txtApi.Text, (int)numdelayclickmin.Value, (int)numdelayclickmax.Value, checkBox1.Checked, cboWebsite.Text, token);
            ...
            { }
            File.WriteAllLines(txtEmail.Text, ls_email);
        }
